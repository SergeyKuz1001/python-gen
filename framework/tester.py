...
class Tester:
    ...
    def codefilename(self) -> str:
        if not hasattr(self, "_codefilename"):
            object.__setattr__(self, "_codefilename", path.join(self.cache_dir, self.generator.identifier + ".py"))
        return self._codefilename

    @property
    def statfilename(self) -> str:
        if not hasattr(self, "_statfilename"):
            object.__setattr__(self, "_statfilename", path.join(self.cache_dir, self.generator.identifier + "." + self.adapter.identifier + "-accepted"))
        ...
        if path.isfile(self.statfilename):
            ...
            for rule_checker in self.generator.rule_checkers:
                if not rule_checker.in_rules(rules):
                    print(f"Rules match failure for checker {rule_checker.identifier}, rules = {rules}")
                    return False
