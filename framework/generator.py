...
class Generator(CSTTransformer):
    ...
    def __init__(
        self, seed: int, grammar: "Grammar", checkers: list[AbstractChecker]
    ):
    ...
    def code(self) -> str:
        if self._code is None:
            ...
            object.__setattr__(
                self, "_code", self.grammar.start.visit(self).code
            )
        return self._code
    ...
    def rule_checkers(self) -> list[RuleChecker]:
        return list(
            filter(
                lambda checker: isinstance(checker, RuleChecker), self.checkers
            )
        )
    ...
    def identifier(self) -> str:
        return (
            self.grammar.identifier
            + "_"
            + str(self.choose.seed)
            + "".join(
                map(lambda checker: "_" + checker.identifier, self.checkers)
            )
        )
    ...
    def on_trace(self, *local_filters) -> bool:
        if isinstance(local_filters[0], GlobalFilter):
            ...
            global_filter = (
                ...
                if all(
                    map(
                        lambda lf: type(lf) == str
                        or isinstance(lf, AbstractFilter),
                        local_filters,
                    )
                )
                else AllNodes()
    ...
    def on_visit(self, node: CSTNodeT) -> bool:
        if isinstance(node, GenNode):
            ...
            raise TypeError(
                f"Argument isn't node because it has type {type(node)}"
            )
    ...
    def _trans_add(self, func: Callable) -> None:
        self._trans_list[-1].append(func)

    def _trans_fix(self) -> None:
        self._trans_list.pop()

    def _trans_rollback(self) -> None:
        funcs = self._trans_list.pop()
    ...
    def _on_visit_mid_node(self, mid_node: CSTNodeT) -> None:
        for checker in self.checkers:
            self._trans_add(
                lambda checker=checker: checker.on_unvisit_mid_node(
                    mid_node, self
                )
            )
            checker.on_visit_mid_node(mid_node, self)

    def _on_leave_mid_node(
        self, mid_node: CSTNodeT, new_node: CSTNodeT
    ) -> None:
        for checker in reversed(self.checkers):
            self._trans_add(
